...
func TestDailyLogWriterCreatesNewFileAndRotatesByDay(t *testing.T) {
	...
	writer, err := newDailyLogWriter(dir, "phonecodeworker", func() time.Time { return now })
	if err != nil {
	...
	if !strings.Contains(filepath.Base(firstPath), "phonecodeworker-20260616-010203") {
	...
	secondWriter, err := newDailyLogWriter(dir, "phonecodeworker", func() time.Time { return now })
	if err != nil {
	...
	if !strings.Contains(filepath.Base(rotatedPath), "phonecodeworker-20260617-000001") {
