...
func TestSaveSucceededImportFileOnlyIncludesSucceededRecords(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "phones.success.txt")
	state := NewState("/tmp/phones.txt", "https://code.test/?phone=", []string{
		"18507561351",
		"18507561352",
		"18507561353",
	...
	state.Records[0].Status = recordStatusSucceeded
	state.Records[0].UpdatedAt = time.Now()
	state.Records[1].Status = recordStatusCreated
	state.Records[1].TaskID = 9
	state.Records[2].Status = recordStatusPending
	state.Records[3].Status = recordStatusFailed

	if err := SaveSucceededImportFile(path, state); err != nil {
		t.Fatalf("save succeeded import file: %v", err)
	}
	raw, err := os.ReadFile(path)
	if err != nil {
