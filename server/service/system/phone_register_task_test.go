...
func TestPhoneRegisterTaskLeaderSummaryAggregatesByDeputyCreator(t *testing.T) {
	setupPhoneRegisterTaskTestDB(t)
	...
	for _, promoterID := range []uint{31, 32, 33, 34} {
		require.NoError(t, global.GVA_DB.Create(&modelSystem.SysPhoneRegisterTask{
			...
			SMSReceiveMode: modelSystem.PhoneRegisterSMSModePlatformSend, Status: modelSystem.PhoneRegisterStatusSucceeded,
			StatusCode: &successCode, FinishedAt: &now, ExpiresAt: now.Add(time.Hour),
		}).Error)
	}
	failCode := modelSystem.PhoneRegisterStatusCodeDeviceExecFail
	require.NoError(t, global.GVA_DB.Create(&[]modelSystem.SysPhoneRegisterTask{
		{Phone: "18800000041", PromoterID: 31, LeaderID: &leaderID, SMSReceiveMode: modelSystem.PhoneRegisterSMSModePlatformSend, Status: modelSystem.PhoneRegisterStatusFailed, StatusCode: &failCode, FinishedAt: &now, ExpiresAt: now.Add(time.Hour)},
		{Phone: "18800000042", PromoterID: 32, LeaderID: &leaderID, SMSReceiveMode: modelSystem.PhoneRegisterSMSModePlatformSend, Status: modelSystem.PhoneRegisterStatusPending, ExpiresAt: now.Add(time.Hour)},
		{Phone: "18800000043", PromoterID: 33, LeaderID: &leaderID, SMSReceiveMode: modelSystem.PhoneRegisterSMSModePlatformSend, Status: modelSystem.PhoneRegisterStatusSucceeded, StatusCode: &successCode, FinishedAt: &now, SettledAt: &now, ExpiresAt: now.Add(time.Hour)},
	}).Error)
...
func TestPhoneRegisterRiskReducesRepeatPromoterProbability(t *testing.T) {
	got := runPhoneRegisterRepeatRiskCase(t, 0.5)
...
func TestPhoneRegisterRiskReductionStillAllowsRepeatHit(t *testing.T) {
	got := runPhoneRegisterRepeatRiskCase(t, 0.1)
...
func runPhoneRegisterRepeatRiskCase(t *testing.T, randomValue float64) modelSystem.SysPhoneRegisterTask {
	t.Helper()
	setupPhoneRegisterTaskTestDB(t)
	restore := stubPhoneRegisterRiskRandom(randomValue)
	t.Cleanup(restore)
	...
	riskCode := modelSystem.PhoneRegisterStatusCodeRiskFace
	recentRiskTask := modelSystem.SysPhoneRegisterTask{
		Phone:          "18800000301",
		PromoterID:     promoterAID,
		LeaderID:       &leaderID,
		SMSReceiveMode: modelSystem.PhoneRegisterSMSModePlatformSend,
	...
	require.NoError(t, global.GVA_DB.Create(&recentRiskTask).Error)
	...
	holderDeviceID := fmt.Sprintf("openapi-repeat-risk-device-%.1f", randomValue)
	task := modelSystem.SysPhoneRegisterTask{
		Phone:          "18800000303",
		PromoterID:     promoterAID,
		LeaderID:       &leaderID,
